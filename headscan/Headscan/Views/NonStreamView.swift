    ...
    ZStack {
      ...
      VStack {
        ...
        VStack(spacing: 12) {
          ...
          Text("Watch Your Glasses' Point of View")
            .font(.system(size: 20, weight: .semibold))
            .foregroundStyle(.white)

          Text("Tap the Start streaming button to watch a live feed from your glasses' camera.")
            .font(.system(size: 15))
...
struct UpdateRequiredMessage: View {
  ...
  private var message: String {
    if showFirmwareUpdate && showDATAppUpdate {
      return "Your glasses firmware and app need updates before Headscan can start."
    }
    if showFirmwareUpdate {
      return "Your glasses firmware needs an update before Headscan can start."
    }
    return "The app on your glasses needs an update before Headscan can start."
  }
  ...
  var body: some View {
    VStack(spacing: 24) {
      ...
      VStack(spacing: 12) {
        TipItemView(
          resource: .videoIcon,
          text: "First, Headscan needs permission to use your glasses camera."
        )
        TipItemView(
